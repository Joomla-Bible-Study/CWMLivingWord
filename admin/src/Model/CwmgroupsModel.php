 ...
 * @since  5.7.0
 */
class CwmgroupsModel extends ListModel
{
    /**
    ...
    protected function getListQuery(): QueryInterface
    {
        ...
        $query->select(
            $this->getState(
                ...
                implode(', ', $db->quoteName([
                    'a.id', 'a.name', 'a.description', 'a.plan_id', 'a.start_date',
                    'a.published', 'a.checked_out', 'a.checked_out_time', 'a.ordering',
                ]))
            )
        );
        $query->from($db->quoteName('#__livingword_groups', 'a'));
        ...
        // Subselect member count
        $memberCountQuery = $db->getQuery(true)
            ->select('COUNT(*)')
            ->from($db->quoteName('#__livingword_group_members', 'gm'))
            ->where($db->quoteName('gm.group_id') . ' = ' . $db->quoteName('a.id'));
        $query->select('(' . $memberCountQuery . ') AS ' . $db->quoteName('member_count'));
