...
namespace CWM\Component\Livingword\Administrator\View\Cwmlinks;
...
class HtmlView extends BaseHtmlView
{
    ...
    protected function addToolbar(): void
    {
        ...
        ToolbarHelper::title(Text::_('COM_LIVINGWORD_MANAGE_LINKS'), 'link');

        if ($this->canDo->get('core.create')) {
            $toolbar->addNew('cwmlink.add');
        }
        ...
        if ($this->canDo->get('core.edit.state')) {
            $childBar->publish('cwmlinks.publish');
            $childBar->unpublish('cwmlinks.unpublish');
            $childBar->checkin('cwmlinks.checkin')->listCheck(true);

            if ((int) $this->state->get('filter.published') !== ContentComponent::CONDITION_TRASHED) {
                $childBar->trash('cwmlinks.trash')->listCheck(true);
            }
        }
        ...
        if (
            ...
            $toolbar->delete('cwmlinks.delete')
